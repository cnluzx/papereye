...
- `run.bat` 和 `paper-eye.ps1` 位于同一目录。
- `README.md` 中的功能说明与当前界面一致。
- 在 Windows 10/11 上双击 `run.bat` 能正常启动。
- 托盘菜单、快捷键、自启动、语言切换都经过手动验证。
- 如果后续分发二进制安装包，再补充安装器和卸载说明。

## 隐私说明

PaperEye 只在本地运行，不联网，不上传屏幕内容，不采集用户数据。配置仅保存到当前用户的 `%APPDATA%\PaperEye` 目录。

## 许可证

如果你准备公开发布，建议在仓库中补充 `LICENSE` 文件，例如 MIT License。
